<nav class="bg-instansi-dark text-white sticky top-0 z-50 bg-footer border-b border-blue-900/40 shadow-md">
    ...
    <div class="w-full px-6 sm:px-12 py-3 flex items-center justify-between">
        ...
        <!-- ======================================================= -->
        <!-- MENU NAVBAR UNTUK LAPTOP (SUDAH DIURUTKAN) -->
        <!-- ======================================================= -->
        <div class="hidden md:flex items-center space-x-6 text-sm font-medium">
            ...
            <a href="{{ url('/standar-pelayanan') }}" class="{{ request()->is('standar-pelayanan*') ? 'text-amber-400 font-bold' : 'text-gray-300 hover:text-amber-400' }} transition">Standar Pelayanan Publik</a>
            
            <!-- 4. Layanan (Dropdown Laptop) -->
            <div class="relative">
                <button id="desktop-dropdown-btn" class="{{ request()->is('kunjungan*') || request()->is('layanan-pengaduan*') || request()->is('layanan-integrasi*') ? 'text-amber-400 font-bold' : 'text-gray-300 hover:text-amber-400' }} flex items-center gap-1 transition focus:outline-none">
                    Layanan
            ...
            <!-- 9. Produk Karya WBP -->
            <a href="{{ url('/produk-wbp') }}" class="{{ request()->is('produk-wbp*') ? 'text-amber-400 font-bold' : 'text-gray-300 hover:text-amber-400' }} transition">Produk Karya WBP</a>
        </div>
...
<!-- ======================================================= -->
<!-- SIDEBAR MENU UNTUK HP (SUDAH DIURUTKAN) -->
<!-- ======================================================= -->
<div id="sidebar-menu" class="fixed inset-y-0 right-0 w-64 bg-[#0a1e36] text-white shadow-2xl z-50 transform translate-x-full transition-transform duration-300 ease-in-out md:hidden">
    ...
    <div class="flex flex-col space-y-4 px-6 py-4 text-base font-semibold">
        ...
        <!-- 4. Layanan (Dropdown HP) -->
        <div class="w-full">
            <button id="mobile-dropdown-btn" class="w-full text-left {{ request()->is('kunjungan*') || request()->is('layanan-pengaduan*') || request()->is('layanan-integrasi*') ? 'text-amber-400 font-bold' : 'text-gray-300 hover:text-amber-400' }} flex items-center justify-between transition focus:outline-none">
                Layanan
